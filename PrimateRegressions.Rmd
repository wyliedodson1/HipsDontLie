...
# Your turn:
lmFemurHead <- lm(AbductedKnee ~ FemurHead, data = pri.dat)
lmNSA <- lm(AbductedKnee ~ NSA, data = pri.dat)
lmNeck <- lm(AbductedKnee ~ Neck, data = pri.dat)
lmL2 <- lm(AbductedKnee ~ L2, data = pri.dat)
lmL4 <- lm(AbductedKnee ~ L4, data = pri.dat)
lmFTA <- lm(AbductedKnee ~ FTA, data = pri.dat)
lmPS <- lm(AbductedKnee ~ PS, data = pri.dat)
lmPI <- lm(AbductedKnee ~ PI, data = pri.dat)
lmAS <- lm(AbductedKnee ~ AS, data = pri.dat)
lmAI <- lm(AbductedKnee ~ AI, data = pri.dat)
lmTotalArea <- lm(AbductedKnee ~ TotalArea, data = pri.dat)
lmSUP <- lm(AbductedKnee ~ SUP, data = pri.dat)
lmANT <- lm(AbductedKnee ~ ANT, data = pri.dat)
lmAcetabDiameter <- lm(AbductedKnee ~ AcetabDiameter, data = pri.dat)
lmPelvicBreadth <- lm(AbductedKnee ~ PelvicBreadth, data = pri.dat)
...
summary(lmFemurHead)# 3b. Which of your models show a significant relationship between the ROM (y) variable and the morphology (x) variable (p value in the x variable row is LESS THAN 0.05)?
```{r}
# Femur Head, NSA, Neck, L2, L4, FTA, PS, PI, AS, AI, Total Area, SUP, ANT, Acetabular Diameter, Pelvic Breadth. 
```
...
```{r}
# Femur Head, Neck, L2, L4, PS, PI, AS, AI, Total Area, Acetabular Diameter, Pelvic Breadth 
```
...
# Your turn:
plot(pri.dat$FemurHead, pri.dat$AbductedKnee)
abline(lmFemurHead) 

plot(pri.dat$NSA, pri.dat$AbductedKnee)
abline(lmNSA)

plot(pri.dat$Neck, pri.dat$AbductedKnee)
abline(lmNeck)

plot(pri.dat$L2, pri.dat$AbductedKnee)
abline(lmL2)

plot(pri.dat$L4, pri.dat$AbductedKnee)
abline(lmL4)

plot(pri.dat$FTA, pri.dat$AbductedKnee)
abline(lmFTA)

plot(pri.dat$PS, pri.dat$AbductedKnee)
abline(lmPS)

plot(pri.dat$PI, pri.dat$AbductedKnee)
abline(lmPI)

plot(pri.dat$AS, pri.dat$AbductedKnee)
abline(lmAS)

plot(pri.dat$AI, pri.dat$AbductedKnee)
abline(lmAI)

plot(pri.dat$TotalArea, pri.dat$AbductedKnee)
abline(lmTotalArea)

plot(pri.dat$SUP, pri.dat$AbductedKnee)
abline(lmSUP)

plot(pri.dat$ANT, pri.dat$AbductedKnee)
abline(lmANT)

plot(pri.dat$AcetabDiameter, pri.dat$AbductedKnee)
abline(lmAcetabDiameter)

plot(pri.dat$PelvicBreadth, pri.dat$AbductedKnee)
...
```{r}
# Femur Head, Neck, L2, Acetabular Diameter, Pelvic Breadth 
```
...
```{r}
# Femur Head, Neck, L2, Acetabular Diameter, and Pelvic Breadth, which are all positive 
```
...
```{r}
# All of the answers to number 5 are also answers to number 3, so yes they are the same. 
```
...
```{r}
AbK_best <- lm(pri.dat$AbductedKnee ~ pri.dat$FemurHead + pri.dat$Neck + pri.dat$L2 + pri.dat$AcetabDiameter + pri.dat$PelvicBreadth, data = pri.dat)
```
